...
def _parse_entso_e_xml(xml_text: str, period_start: datetime, period_end: datetime) -> pd.Series:
    ...
    if not records:
        log.warning(
            "ENTSO-E returned no price points for %s – %s",
...
def fetch_entso_e_prices(
    ...
    log.info(
        ...
        len(hourly), len(df), int(gaps_before),
    )
...
def fetch_open_meteo_weather(
    ...
    if "hourly" not in data:
        raise RuntimeError(f"Open-Meteo response missing 'hourly' key: {data.get('reason', 'unknown')}")
    ...
    if remaining_gaps > 0:
        log.warning("Open-Meteo: %d NaN cells after interpolation — forward-filling", remaining_gaps)
        df_5min = df_5min.ffill().bfill()

    log.info(
        "Open-Meteo: %d hourly rows → %d 5-min rows",
        len(df_hourly), len(df_5min),
    )
