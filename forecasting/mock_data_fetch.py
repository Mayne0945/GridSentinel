...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s | %(levelname)-8s | %(message)s", stream=sys.stdout
)
...
def run_mock_generation():
    ...
    daily_cycle = 20 * np.sin(np.pi * (hour_of_day - 6) / 12) + 15 * np.sin(
        np.pi * (hour_of_day - 14) / 6
    )
    ...
    df_weather = pd.DataFrame(
        {"temperature_c": temp, "solar_irradiance_wm2": solar, "wind_speed_kmh": wind}, index=idx
    )
