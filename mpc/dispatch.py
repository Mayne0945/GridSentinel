...
def _synthetic_fleet() -> list[BusState]:
    ...
    for i in range(100):
        ...
        buses.append(BusState(
            bus_id=f"Bus_{i+1:03d}", soc=soc, soh=soh,
            capacity_kwh=DEFAULT_CAPACITY_KWH,
            max_charge_kw=DEFAULT_CHARGE_RATE_KW,
            max_discharge_kw=DEFAULT_DISCHARGE_RATE_KW,
            departure_step=departure_step, required_soc=required_soc,
            available=True, is_trusted=True,
        ))
    return buses
...
class MPCDispatcher:
    ...
    def solve(
        ...
        log.info(
            ...
            len(active), HORIZON_STEPS, HORIZON_STEPS * STEP_HOURS, prices_point[0],
        )
    ...
    def _build_and_solve(self, prices_lower, prices_point, buses, t0):
        ...
        P_ch  = pulp.LpVariable.dicts("P_ch",  [(b,t) for b in range(B) for t in range(T)], lowBound=0)
        P_dis = pulp.LpVariable.dicts("P_dis", [(b,t) for b in range(B) for t in range(T)], lowBound=0)
        SoC   = pulp.LpVariable.dicts("SoC",   [(b,t) for b in range(B) for t in range(T+1)], lowBound=SOC_MIN, upBound=SOC_MAX)
        ...
        obj_deg     = pulp.lpSum(buses[b].deg_cost * (P_ch[b,t] + P_dis[b,t]) * STEP_HOURS
                                 for b in range(B) for t in range(T))
        obj_simult  = pulp.lpSum(SIMULTANEOUS_PENALTY * (P_ch[b,t] + P_dis[b,t])
                                 for b in range(B) for t in range(T))
        ...
        for t in range(T):
            prob += P_net[t] == pulp.lpSum(P_ch[b,t] - P_dis[b,t] for b in range(B)), f"P_net_t{t}"
            prob += P_net[t] <=  self.transformer_limit_kw, f"TFmr_import_t{t}"
            prob += P_net[t] >= -self.transformer_limit_kw, f"TFmr_export_t{t}"

        log.info("Solving LP | variables=%d | constraints=%d …",
        ...
        log.info("Solver done | status=%s | solve_time=%.2fs | total=%.2fs",
                 status, solve_time, total_time)

        if prob.status not in (pulp.LpStatusOptimal, 1):
            log.error("Non-optimal: %s", status)
            return self._infeasible_result(t0, status)

        return self._extract_result(prob, buses, P_ch, P_dis, SoC, P_net,
                                    prices_lower, prices_point, solve_time, total_time, status)

    def _extract_result(self, prob, buses, P_ch, P_dis, SoC, P_net,
                        prices_lower, prices_point, solve_time, total_time, status):
        T = HORIZON_STEPS
        B = len(buses)
        ...
        for b, bus in enumerate(buses):
            ...
            for t in range(T):
                ...
                schedule.append({"step": t, "action": action,
                                 "charge_kw": round(ch,2), "discharge_kw": round(dis,2),
                                 "soc": round(sn,4)})
            ...
            if ch0 > dis0:
                action = "charge"; power_kw = ch0
                reason = self._charge_reason(bus, prices_point[0])
                total_ch_t0 += ch0
            elif dis0 > ch0:
                action = "discharge"; power_kw = dis0
                reason = f"arbitrage — price {prices_lower[0]:.1f} EUR/MWh"
                total_dis_t0 += dis0
            else:
                action = "hold"; power_kw = 0.0
                reason = self._hold_reason(bus)

            commands.append(BusCommand(bus_id=bus.bus_id, action=action,
                                       power_kw=round(power_kw,2),
                                       soc_after=round(soc1,4), reason=reason))
        ...
        log.info("Dispatch t=0 | charge=%.0fkW (%d) | discharge=%.0fkW (%d) | "
                 "hold=%d | transformer=%.1f%% | profit=€%.2f",
                 total_ch_t0, counts["charge"], total_dis_t0, counts["discharge"],
                 counts["hold"], tfmr_pct, profit)

        return DispatchResult(
            generated_at=datetime.now(timezone.utc).isoformat(),
            horizon_steps=T, step_minutes=STEP_MINUTES,
            solver_status=status, solve_time_s=round(solve_time,3),
            ...
            commands=commands, full_schedule=full_schedule,
            meta={
    ...
    def _hold_reason(self, bus):
        if bus.soc >= SOC_STRESS_HIGH: return "SoC at ceiling — avoiding stress degradation"
        if bus.soc <= SOC_STRESS_LOW:  return "SoC at floor — protecting battery health"
        return "no profitable action this step"

    def _infeasible_result(self, t0, reason):
        return DispatchResult(
            generated_at=datetime.now(timezone.utc).isoformat(),
            horizon_steps=HORIZON_STEPS, step_minutes=STEP_MINUTES,
            solver_status="Infeasible", solve_time_s=round(time.perf_counter()-t0,3),
            objective_value=0.0, expected_profit_eur=0.0,
            total_discharge_kw=0.0, total_charge_kw=0.0,
            transformer_load_pct=0.0, commands=[], full_schedule={},
            meta={"reason": reason},
        )
...
def _build_arg_parser():
    p = argparse.ArgumentParser(description="GridSentinel MPC — Dispatch Brain",
                                formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    p.add_argument("--forecast", "-f", type=Path, default=Path("data/forecasts/latest_forecast.json"))
...
def main():
    ...
    if not args.forecast.exists():
        raise FileNotFoundError(f"Forecast not found: {args.forecast}\nRun forecasting/inference.py first.")
    ...
    dispatcher = MPCDispatcher(transformer_limit_kw=args.transformer_limit,
    ...
    print("     Next: python digital_twin/validate.py --dispatch data/dispatch/latest_dispatch.json")
    print("═" * 60 + "\n")
